...
'''What historical data are we going to use? We are only using for one day, should we say "a typical day in Algeria
in the summer" by approx historical data from past say 10 years for months Jun-Aug?'''
...
plt.text((x.min() + x.max())/2, y.min() + (y.max() - y.min())*0.2,  # Middle horizontally, 10% from bottom
         equation_text, 
         color="blue", 
         fontsize=12,
